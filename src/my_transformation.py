...
bronze_schema = StructType([
    ...
    StructField("device", StructType([StructField("os", StringType(), True), StructField("browser", StringType(), True)]), True),
    StructField("geo", StructType([StructField("country", StringType(), True), StructField("city", StringType(), True)]), True),
...
# Using @dp.temporary_view to avoid cluttering the Unity Catalog with staging logic.
...
# 'cluster_by_auto=True' enables Databricks to dynamically re-organize data layout 
# based on query filters, significantly reducing I/O for dashboard queries.
@dp.table(
    name="fact_clicks",
...
def fact_clicks():
    return dp.read_stream("silver_events").select(
        "event_id", "user_id_hashed", "event_type", "event_timestamp",
        expr("CAST(event_timestamp AS DATE) AS date_key"), "city", "country"
    )

# --- 7. GOLD AGGREGATIONS: Business Reporting ---
# Final high-level metrics updated incrementally as new events flow through.
@dp.table(
    name="gold_daily_geo_stats",
    cluster_by_auto=True
)
...
def gold_final_report():
    return (
        dp.read("gold_daily_geo_stats").alias("stats")
